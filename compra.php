                ...
                foreach ($objs as $o):
                    ...
                    <tr>
                        ...
                        <?php
                            ...
                            if(is_array($guias)){
                                foreach($guias as $g){
                                    $txt .= "<a href='movimiento_producto_guia_old.php?id=".$g["id"]."&numero=".$g["numero_guia"]."'>".$g["numero_guia"]."</a>,";
                                }
                                $txt = substr($txt,0,-1);
                                echo $txt;
                                echo "<br/>";
                            ...
                            <div class='control-group col-md-12'>
                                ...
                                <select name="numero_guia" id="numero_guia" class="form-control">
                                    ...
                                    $guias = $objconn->consulta_matriz(
                                        "SELECT gp.id, numero_guia, date_format(fecha_registro, '%d-%m-%Y') as fecha_registro FROM guia_producto gp WHERE gp.numero_guia NOT IN(
                                                        SELECT gp.numero_guia FROM compra_guia cg
                                                        INNER JOIN guia_producto gp ON cg.id_guia_producto = gp.id ) AND gp.tipo=1 ");
                                    foreach ($guias as $guia): ?>
                                        <option value="<?= $guia['id']; ?>"><?= "({$guia['id']})  - {$guia['numero_guia']} - [{$guia['fecha_registro']}]"; ?></option>
                                    <?php endforeach; ?>
                                </select>
