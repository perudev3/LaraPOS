...
class producto extends SuperClass { 
    ...
    public function ServerSideCotos($request)
    {
      ...
      where mp.estado_fila = 1 and p.estado_fila=1 and mp.fecha_cierre <= '$now' ";
      ...
      //     GROUP BY p.id";
      $sqlCantidad="SELECT
      p.id, p.nombre, CONCAT('S/. ',p.precio_compra) AS precio_compra, 
      CONCAT('S/. ', p.precio_venta) AS precio_venta, CASE WHEN p.incluye_impuesto = 1 THEN 'SI' ELSE 'NO' END 
      AS incluye_impuesto, 
      ...
      if(!empty($search)){
        ...
        $aux .= " OR precio_compra LIKE '%$search%'";
        $aux .= " OR precio_venta LIKE '%$search%'";
        $aux .= " OR p.nombre LIKE '%$search%'";
        $aux .= " OR a.nombre LIKE '%$search%'";
        $aux .= " )";
        $sql .= $aux;
        // echo $sql;
        // $sqlCantidad .= " WHERE 1 = 1 ".$aux."";
        $sqlCantidad .= "".$aux." GROUP BY mp.id_producto,a.id HAVING stock>0";
      }else{
        $sqlCantidad .= "GROUP BY mp.id_producto,a.id HAVING stock>0 ";
      }
      // var_dump($sqlCantidad);
      $sql .= " GROUP BY mp.id_producto,a.id HAVING stock>0 ORDER BY id DESC LIMIT ".$start.",".$length."";

      //echo($sql);
      

      // $totalData = parent::consulta_arreglo($sqlCantidad)[0];
      $totalData = parent::consulta_cantidad_cotos($sqlCantidad);
      ...
      if(!empty($result)){
        foreach ($result as $item) {
         ...
         /*  $subdata[] = convert_cotos($item[0]);
            ...
            $subdata[]=iconv(mb_detect_encoding($item[4],"UTF-8,ISO-8859-1"),'UTF-8',$item[4]);
            $subdata[]=iconv(mb_detect_encoding($item[5],"UTF-8,ISO-8859-1"),'UTF-8',$item[5]);
            $subdata[]=iconv(mb_detect_encoding($item[6],"UTF-8,ISO-8859-1"),'UTF-8',$item[6]);
            $subdata[]=iconv(mb_detect_encoding($item[7],"UTF-8,ISO-8859-1"),'UTF-8',$item[7]);
