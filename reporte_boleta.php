...
$titulo_pagina = 'Reporte de Boletas';
...
<body>
    ...
    $objs = $conn->consulta_matriz("SELECT v.id , b.id_venta, v.subtotal, v.total_impuestos, v.total,v.tipo_comprobante, v.fecha_hora,v.fecha_cierre,v.id_turno,v.id_usuario, v.id_caja, v.id_cliente,v.estado_fila,b.id as id_boleta FROM venta v inner join boleta b on (v.id=b.id_venta) where v.fecha_cierre between '" . $fechaInicio . " 00:00:00' AND '" . $fechaFin . " 23:59:59'");
            ...
            <tbody>
                ...
                if (is_array($objs)) :
                    ...
                    foreach ($objs as $o) :
                        ?>
                        <tr>
                            <td style="text-align: center;"><b><?php echo $config['serie_boleta'] . '-' . str_pad($o['id_boleta'], 8, "0", STR_PAD_LEFT); ?></b></td>
                            ...
                            <?php
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <a title="Reimprimir" class="btn btn-sm btn-default" href='#' onclick="reimprimir(<?php echo $o['id']; ?>,'BOL',<?php echo $_COOKIE["id_caja"]; ?>)"><i class="fa fa-print" aria-hidden="true"></i></a>
                                        <a title="Descargar Boleta" class="btn btn-sm btn-info" onclick='DescargarBoleta(<?php echo $o['id_boleta'] ?>)'><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>
                                        <a title="Anular Boleta" class="btn btn-sm btn-danger" onclick='anularBoleta(<?php echo $o['id_boleta'] ?>)'><span class='glyphicon glyphicon-remove-circle'></span></a>
                                    </div>
                                </td>
                            <?php
                                    } else
