...

{
    ...
    public function processItemWithSaleData()
    {
        ...
        try {
            ...
            $observaciones =  $this->conection->consulta_arreglo("SELECT observaciones FROM venta WHERE id = '" . $this->idVenta . "' ");
    ...
    public function dataReadyToSendRobaFact($dataVenta, $dataItems, $dataConfiguracion, $cliente)
    {
        ...
        if ($this->expiresDate != "") {
            ...
            $fecha_de_vencimiento = date("d-m-Y", strtotime($fecha_actual . "+ " . $this->expiresDate . " days"));
            $condicion_de_pago = $this->expiresDate . " días para cancelar el crédito";
        }
        ...
        $data = array(
            ...
            "moneda"                            => "1", // variables de monedas
            "tipo_de_cambio"                    => "",
            "porcentaje_de_igv"                 => "18",
            "descuento_global"                  => $this->descuentoGlobal,
            "total_descuento"                   => "",
            "total_anticipo"                    => "", // cuando es credito
            ...
            "medio_de_pago"                     => strtoupper($medios_de_pagos), // enviar medios de pago 
            "placa_vehiculo"                    => "",
        ...
        if (sizeof(json_decode($this->venta_al_credito)) > 0) {
            //$arrCredito = json_decode($this->venta_al_credito);
            $cuotas = array();
            foreach (json_decode($this->venta_al_credito) as $key => $vc) {
                $cuotas[$key] =
                    array(
                        'cuota' => $vc->cuota,
                        'fecha_de_pago' => $vc->fecha_de_pago,
                        'importe' => $vc->importe
                    );
            }

            $data["venta_al_credito"] = $cuotas;
        }
    ...
    public function insertSaleDocument($data, $dataVenta)
    {
        ...
        $total = 0;
        foreach (json_decode($this->medioPago) as $mp) {
            $total += floatval($mp->monto) - floatval($mp->vuelto);
        }
        //var_dump($total);
        $subtotal = $total/1.18;
        $igv = $total - $subtotal;
        if ($this->typeDocument == 1 || $this->typeDocument == 4) { // BOLETA
            ...
            $sql = "UPDATE venta SET id_cliente='" . $this->cliente['id'] . "', tipo_comprobante='1', total='" . $total . "', total_impuestos='" . $igv . "', subtotal='" . $subtotal . "', observaciones='" . $this->observaciones . "' WHERE id=" . $this->idVenta . " ";
            $this->conection->consulta_simple($sql);
        }
        ...
        if ($this->typeDocument == 2 || $this->typeDocument == 5) { // FACTURA
            ...
            $this->conection->consulta_simple($sql);
            $sql = "UPDATE venta SET id_cliente='" . $this->cliente['id'] . "', tipo_comprobante='2', total='" . $total . "', total_impuestos='" . $igv. "', subtotal='" . $subtotal . "', observaciones='" . $this->observaciones . "' WHERE id=" . $this->idVenta . " ";
            $this->conection->consulta_simple($sql);
        }
        ...
        if ($this->typeDocument == 0 || $this->typeDocument == 3) { // NOTA DE VENTA y NOTA DE VENTA CREDITO
            ...
            if ($this->typeDocument == 0) { // NOTA DE VENTA
                ...
                $sql = "UPDATE venta SET id_cliente='" . $this->cliente['id'] . "', tipo_comprobante='0', total='" . $total . "', total_impuestos='" . $igv . "', subtotal='" . $subtotal . "', observaciones='" . $this->observaciones . "' WHERE id=" . $this->idVenta . " ";
        ...
        if (sizeof(json_decode($this->venta_al_credito)) > 0) {
            $sqlCuotas = "";
            foreach (json_decode($this->venta_al_credito) as $vc) {
                $sqlCuotas = "INSERT INTO ventas_cuotas  VALUES('null','" . $this->idVenta . "', '" . $vc->cuota . "'," . $vc->importe . ",'" . date("Y-m-d", strtotime($vc->fecha_de_pago)) . "') ";

                $this->conection->consulta_simple($sqlCuotas);
            }
        }
