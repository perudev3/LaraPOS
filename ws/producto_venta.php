...
if (isset($_POST['op'])) {
    switch ($_POST['op']) {
        ...
        case 'searchCoti':
            $res = $objproducto_venta->consulta_arreglo("SELECT subtotal, total_impuestos, total, fecha_hora, nombres_y_apellidos as usuario, ca.nombre as caja, cli.documento, cli.nombre as cliente, direccion, correo, tiempo_valido  
            FROM cotizacion co
            INNER JOIN cliente cli ON cli.id = co.id_cliente
            INNER JOIN caja ca ON ca.id = co.id_caja
            INNER JOIN usuario u ON u.id = co.id_usuario WHERE co.id = '" . $_POST["id"] . "'");


            $res["detalles"] = $objproducto_venta->consulta_matriz("SELECT dc.id, id_producto as idProducto, nombre, cantidad, precio 
            FROM detalles_cotizacion dc
            INNER JOIN producto p ON p.id = dc.id_producto WHERE id_coti = '" . $_POST["id"] . "'");
