...

{
    ...
    private $medioPago = 0;
    private $idCaja = 0;
    //impuesto a la bolsa - nombre como se registra en productos
    public $ICBPER = 'IMPUESTO BOLSA 2021';
    //monto del impuesto del año 2021
    ...
    public function __construct($cliente, $idVenta, $typeDocument, $descuentoGlobal, $medioPago,$idCaja)
    {
                ...
                if ($proccess['success'] == true) {
                        ...
                        if ($resBoleta['success'] != true) {
                            ...
                            if ($returnCredito == true) {
                                ...
                                if (!$dataCliente == false) {
                                    ...
                                    $retornar = $this->dataReadyToSendRobaFact($proccess['calculosVenta'], $proccess['items'], $dataSerieNumeroTokenRuta, $dataCliente);
                                    /*if ($retornar['success']) {

                                        $verificaImpresion = $this->conection->consulta_arreglo("SELECT * FROM configuracion_impresion WHERE caja='" . $this->idCaja . "' AND opcion='BOL' ");
                                        //$verificaImpresion = $this->conection->consulta_arreglo("SELECT * FROM configuracion_impresion WHERE opcion='BOL' ");
                                        if (isset($verificaImpresion['id'])) {
                                            ...
                                            $boleta = $this->conection->consulta_arreglo("SELECT id, serie FROM boleta WHERE id_venta = {$this->idVenta}");
                                           ...
                                           */ $success = true;
                                           ...
                                           /* $hash = $this->conection->consulta_arreglo("SELECT * FROM new_comprobante_hash WHERE estado=1 AND pkComprobante =" . $boleta['id']);
                    ...
                    if ($this->typeDocument == 2  || $this->typeDocument == 5) { // FACTURA
                        ...
                        if ($resFactura['success'] == false) { // ERROR
                            ...
                            $esCredito = false;
                            $returnCredito = true;
                            if ($this->typeDocument == 5) { // factura CREDITO
                                $returnCredito = new ValidacionCredito($this->conection, $proccess['montototalsindescuento'], $this->cliente['id']);
                            }
                            if ($returnCredito == true) {
                                $dataCliente = $this->madeDataCustomer();
                                if (!$dataCliente == false) {
                                   ...
                                   if ($retornar['success']) {
                                      ...
                                      /*  $verificaImpresion = $this->conection->consulta_arreglo("SELECT * FROM configuracion_impresion WHERE caja='" . $this->idCaja . "' AND opcion='FAC' ");
                                        ...
                                        if (isset($verificaImpresion['id'])) {
                                            ...
                                            $factura = $this->conection->consulta_arreglo("SELECT id, serie FROM factura WHERE id_venta = {$this->idVenta}");
                                            $config = $this->conection->consulta_arreglo("SELECT * from configuracion");
                                            $receipt = "FACTURA ELECTRONICA" . "\n" . $config['serie_factura'] . "-" . str_pad($factura['id'], 8, "0", STR_PAD_LEFT) . "\n\n";
                                            $pos_printer = new PosPrinter($this->idVenta, $printerName, $receipt);
                                           */ $success = true;
                                            $texto = "Representacion impresa de la \n FACTURA DE VENTA ELECTRONICA";
                                           // $hash = $this->conection->consulta_arreglo("SELECT * FROM new_comprobante_hash WHERE estado=1 AND pkComprobante =" . $factura['id']);
...
*/
    ...
    public function dataReadyToSendRobaFact($dataVenta, $dataItems, $dataConfiguracion, $cliente)
    {
        ...
        $data = array(
            "serie_documento"=>$dataConfiguracion['serie'],
            "numero_documento"=>$dataConfiguracion['numero'],
            ...
            "datos_del_emisor"=>
                [                    "codigo_tipo_documento_identidad"=>$cliente['cliente_tipo_de_documento'],
                    "numero_documento"=>$cliente['cliente_numero_de_documento'],
                    "apellidos_y_nombres_o_razon_social"=>$cliente['cliente_denominacion'],
                    "codigo_pais"=>"PE",
                    "ubigeo"=>"",
                    "direccion"=> $cliente['cliente_direccion'],
                    "correo_electronico"=>$cliente['cliente_email'],
                    "telefono"=>"",
                    "codigo_del_domicilio_fiscal"=>""
            ],
            "datos_del_cliente_o_receptor"=>
                ["codigo_tipo_documento_identidad"=>"6",
                "numero_documento"=> $configuracion['ruc'],
                "apellidos_y_nombres_o_razon_social"=> $configuracion['razon_social'],
                "codigo_pais"=>"PE",
                "ubigeo"=>"",
                "direccion"=> $configuracion['direccion'],
                "correo_electronico"=> $configuracion['correoEmisor'],
                "telefono"=> $configuracion['telefono'],
                "codigo_del_domicilio_fiscal"=>""
        ],
            "totales"=>
               [
                    "total_descuentos"=>$this->descuentoGlobal,
                    "total_exportacion"=>0.00,
                    "total_operaciones_gravadas"=> $dataVenta['total_gravada'],
                    "total_operaciones_inafectas"=> $dataVenta['total_inafecta'],
                    "total_operaciones_exoneradas"=>$dataVenta['total_exonerada'],
                    "total_operaciones_gratuitas"=>$dataVenta['total_gratuita'],
                    "total_igv"=>$dataVenta['total_igv'],
                    "total_impuestos"=>0,
                    "total_valor"=>0,
                    "total_venta"=>$dataVenta['montoTotalVenta'],
               ],
            "items"=>[
                
            ] json_encode($dataItems),
            ...
            "motivo_o_sustento_de_nota"=>null,
    ...
    public function insertMovimientoCaja($dataConfiguracion)
    {
        ...
        try {
            ...
            for ($i = 0; $i < count($medio_de_pago); $i++) {
                ...
                if ($this->typeDocument == 3 || $this->typeDocument == 4 || $this->typeDocument == 5) {
                    if ($medio_de_pago[$i]["medio"] === "CREDITO") {
                        ...
                        VALUES (NULL,'" . $this->idCaja . "','" . $monto . "','SELL|PEN|" . $medio_de_pago[$i]["medio"] . "|" . $medio_de_pago[$i]["id"] . "','" . date("Y-m-d H:i:s") . "','" . $dataConfiguracion['fecha_cierre'] . "','" . $miturno['id'] . "','" . $_COOKIE['id_usuario'] . "','1')");
                    } else {
                        $stmt1 = $conn->consulta_simple("Insert into movimiento_caja (id,id_caja,monto,tipo_movimiento,fecha,fecha_cierre,id_turno,id_usuario,estado_fila)
                        VALUES (NULL,'" . $this->idCaja . "','" . $monto . "','EXT|PEN|" . $medio_de_pago[$i]["medio"] . "_COBRO|" . $medio_de_pago[$i]["id"] . "','" . date("Y-m-d H:i:s") . "','" . $dataConfiguracion['fecha_cierre'] . "','" . $miturno['id'] . "','" . $_COOKIE['id_usuario'] . "','1')");
                    }
                } else {
                    $stmt1 = $conn->consulta_simple("Insert into movimiento_caja (id,id_caja,monto,tipo_movimiento,fecha,fecha_cierre,id_turno,id_usuario,estado_fila)
                    VALUES (NULL,'" . $this->idCaja . "','" . $monto . "','SELL|PEN|" . $medio_de_pago[$i]["medio"] . "|" . $medio_de_pago[$i]["id"] . "','" . date("Y-m-d H:i:s") . "','" . $dataConfiguracion['fecha_cierre'] . "','" . $miturno['id'] . "','" . $_COOKIE['id_usuario'] . "','1')");
            ...
            if ($vigilante == false) {
               ...
               /* if ($this->typeDocument == 1 || $this->typeDocument == 4) { // BOELETA                    
                    $verificaImpresion = $conn->consulta_arreglo("SELECT * FROM configuracion_impresion WHERE caja='" . $this->idCaja . "' AND opcion='BOL' ");
                    if (isset($verificaImpresion['id'])) {
                        $stmt22 =  $conn->consulta_simple("INSERT INTO cola_impresion (id,codigo,tipo,caja,aux,estado) VALUES (NULL,'" . $this->idVenta . "','BOL','" . $this->idCaja . "','','1')");
                        if (!$stmt22) {
                            $conn->consulta_simple("ROLLBACK");
                            return false;
                        }
               ...
               /* if ($this->typeDocument == 2 || $this->typeDocument == 5) { // FACTURA
                    $verificaImpresion = $conn->consulta_arreglo("SELECT * FROM configuracion_impresion WHERE caja='" . $this->idCaja . "' AND opcion='FAC' ");
                    if (isset($verificaImpresion['id'])) {
                        $stmt22 = $conn->consulta_simple("INSERT INTO cola_impresion (id,codigo,tipo,caja,aux,estado) VALUES (NULL,'" . $this->idVenta . "','FAC','" . $this->idCaja . "','','1')");
                        if (!$stmt22) {
                            $conn->consulta_simple("ROLLBACK");
                            return false;
                        }
               ...
               /* if ($this->typeDocument == 0 || $this->typeDocument == 3) { // NOTA DE VENTA                                        
                    $verificaImpresion = $conn->consulta_arreglo("SELECT * FROM configuracion_impresion WHERE caja='" . $this->idCaja . "' AND opcion='NOT' ");
                    if (isset($verificaImpresion['id'])) {
                        $stmt22 = $conn->consulta_simple("INSERT INTO cola_impresion (id,codigo,tipo,caja,aux,estado) VALUES (NULL,'" . $this->idVenta . "','NOT','" . $this->idCaja . "','','1')");
                        if (!$stmt22) {
                            $conn->consulta_simple("ROLLBACK");
                            return false;
                        }
    ...
    {
        $productoscomida = $this->conection->consulta_arreglo(
            " select count(*) as existe from producto_venta pv
        ...
        if (!empty($productoscomida)) {
            if ($productoscomida['existe'] != null) {
                if ($productoscomida['existe'] > 0) {
                    $qr = $this->conection->consulta_simple("Insert into cola_impresion values(NULL, {$this->idVenta},'PED', {$this->idCaja},'',1)");
                }
            }
        }
    }
