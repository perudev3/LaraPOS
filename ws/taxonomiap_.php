...
if (isset($_POST['op'])) {
    switch ($_POST['op']) {
        ...
        case 'list':
            ...
            $res = $objtaxonomiap->consulta_matriz("Select DISTINCT prod.id_producto from producto_taxonomiap as prod where (SELECT COUNT(*) from movimiento_producto as 
            mprod where mprod.id_producto = prod.id_producto) > 0 and prod.id_taxonomiap = '".$_POST["tax"]."' AND prod.valor = '".$_POST["valor"]."' LIMIT ".$_POST["offset"].",".$_POST["limit"]."");
            
            if(intval($_POST['id_almacen']) > 0){
                $res = $objtaxonomiap->consulta_matriz("Select DISTINCT prod.id_producto from producto_taxonomiap as prod where (SELECT COUNT(*) from movimiento_producto as 
                mprod where mprod.id_producto = prod.id_producto and mprod.id_almacen = ".$_POST['id_almacen'].") > 0 and prod.id_taxonomiap = '".$_POST["tax"]."' AND prod.valor = '".$_POST["valor"]."' LIMIT ".$_POST["offset"].",".$_POST["limit"]."");
            }
            if (is_array($res)) {
                foreach ($res as &$act) {
                ...
                echo json_encode($_POST['id_almacen']);
            }
            break;
            ...
            $res = $objtaxonomiap->consulta_matriz("Select ptax.id_producto as id_producto from 
            producto_taxonomiap ptax where ptax.valor LIKE '%".$_POST["valor"]."%' AND ptax.id_taxonomiap = 1 
            LIMIT ".$_POST["offset"].",".$_POST["limit"]."");
            
            if(isset($_POST['id_almacen'])){
                $res = $objtaxonomiap->consulta_matriz("Select ptax.id_producto as id_producto from 
                producto_taxonomiap ptax where (SELECT COUNT(*) from movimiento_producto as mprod 
                where mprod.id_producto = ptax.id_producto and mprod.id_almacen = ".$_POST['id_almacen'].") > 0 
                and ptax.valor LIKE '%".$_POST["valor"]."%' AND ptax.id_taxonomiap = 1 
                LIMIT ".$_POST["offset"].",".$_POST["limit"]."");
            }
            ...
            $res = $objtaxonomiap->consulta_matriz("SELECT DISTINCT prod.id_producto FROM producto_taxonomiap prod WHERE (SELECT COUNT(*) from movimiento_producto as 
            mprod where mprod.id_producto = prod.id_producto and mprod.id_almacen = ".$_POST['id_almacen'].") > 0 and prod.valor = '{$_POST["valor"]}' ORDER BY prod.id_producto LIMIT 1");
