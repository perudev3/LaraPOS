...
if (isset($_POST['op'])) {
    switch ($_POST['op']) {
        ...
        case 'addGuia':
            ...
            $coorrelativoremision = $objventa->consulta_arreglo("SELECT max(correlativo) as max FROM guia_remision");
            var_dump($coorrelativoremision);
		
            if($coorrelativoremision[0] == ""){
                $coorrelativoremision = 1;
                $correlativo = $coorrelativoremision;
            }else{
                $coorrelativoremision = $coorrelativoremision[0] + 1;
                    $correlativo = $coorrelativoremision;
            }

            $objventa->consulta_simple("Insert into guia_remision values(NULL,'" . $_POST['id'] . "','" . $_POST['origen'] . "','" . $_POST['destino'] . "','" . $_POST['motivo'] . "','". $correlativo."','" . $_POST['cd'] . "','" . $_POST['cn'] . "')");
                            ...
                            if (!isset($n["errors"])) {
                                ...
                                if (is_array($pagos)) {
                                    ...
                                    foreach ($pagos as $pa) {
                                        ...
                                        if ($pa["medio"] != "DESCUENTO") {
                                            $objconn->consulta_simple("Insert into movimiento_caja values(NULL,'" . $objventa->getIdCaja() . "','-" . $monto . "','SELL|" . $pa["moneda"] . "|" . $pa["medio"] . "','" . date("Y-m-d H:i:s") . "','" . $objventa->getFechaCierre() . "','" . $objventa->getIdTurno() . "','" . $objventa->getIdUsuario() . "','2')");
                                        }
                                ...
                                if (is_array($productos)) {
                                    foreach ($productos as $pr) {
                                        ...
                                        $objconn->consulta_simple("Insert into movimiento_producto values(NULL,'" . $pr["id_producto"] . "','1','" . $pr["cantidad"] . "',NULL,'ANULACION','" . $objventa->getIdUsuario() . "','" . $objventa->getIdTurno() . "','" . date("Y-m-d H:i:s") . "','" . $objventa->getFechaCierre() . "','1','','','1',NULL)");
                                    ...
                                    foreach ($servicios as $ser) {
                                        ...
                                        if (is_array($proxser)) {
                                            foreach ($proxser as $ps) {
                                                $objconn->consulta_simple("Insert into movimiento_producto values(NULL,'" . $ps["id_producto"] . "','" . $ps["id_almacen"] . "','" . $ps["cantidad"] . "',NULL,'ANULACION','" . $objventa->getIdUsuario() . "','" . $objventa->getIdTurno() . "','" . date("Y-m-d H:i:s") . "','" . $objventa->getFechaCierre() . "','1','','','0',NULL)");
        ...
        case 'foul':
            ...
            for ($i = 0; $i < count($medio_de_pago); $i++) {
                ...
                $objventa->consulta_simple("Insert into movimiento_caja values(NULL,'" . $objventa->getIdCaja() . "','-" . $monto . "','SELL|PEN|" . $medio_de_pago[$i]["medio"] . "','" . date("Y-m-d H:i:s") . "','" . $objventa->getFechaCierre() . "','" . $objventa->getIdTurno() . "','" . $objventa->getIdUsuario() . "','2')");
            }
        ...
        case 'AnulaCredito':
            ...
            for ($i = 0; $i < count($medio_de_pago); $i++) {
                ...
                $objventa->consulta_simple("Insert into movimiento_caja values(NULL,'" . $objventa->getIdCaja() . "','-" . $medio_de_pago[$i]["monto"] . "','EXT|PEN|" . $medio_de_pago[$i]["medio"] . "_COBRO','" . date("Y-m-d H:i:s") . "','" . $objventa->getFechaCierre() . "','" . $objventa->getIdTurno() . "','" . $objventa->getIdUsuario() . "','2')");
            }
        ...
        case 'Cotizador':
            ...
            $existCoti = $objventa->consulta_arreglo("SELECT * FROM cotizacion WHERE id =" . $_POST['nro']);

            if (is_array($existCoti)) {

                $objventa->consulta_simple("UPDATE cotizacion
                SET
                subtotal = '" . $_POST["SubTotal"] . "',
                total_impuestos =  '" . $_POST['IGV'] . "',
                total =  '" . $_POST['Total'] . "',
                fecha_hora = '" . date("Y-m-d H:i:s") . "',
                id_usuario = '" . $_POST['idUsuario'] . "',
                id_caja = '" . $_POST["idCaja"] . "',
                id_cliente ='" . $cliente["id"] . "',
                tiempo_valido =  '" . $_POST["tiempo"] . "',
                estado_fila = 1
                WHERE id =  " . $_POST['nro']);

                $details = json_decode($_POST['data']);

                $objventa->consulta_simple("DELETE FROM detalles_cotizacion WHERE id_coti = ".$_POST['nro']);
                foreach ($details as $value) {


                    /*if (!empty($value->id)) {
                        $resp = $objventa->consulta_simple("UPDATE detalles_cotizacion
                    SET
                    ...
                    WHERE id = " . $_POST['nro']);
                        ...
                        VALUES('','" . $_POST['nro'] . "','" . $value->idProducto . "','$value->cantidad','$value->precio')");
                    //}
                }
                ...
                $coti = $objventa->consulta_simple("INSERT INTO cotizacion(id, subtotal, total_impuestos, total, fecha_hora, 
                    id_usuario, id_caja, id_cliente, tiempo_valido, estado_fila) VALUES ('',
                    '" . $_POST["SubTotal"] . "',
                    '" . $_POST['IGV'] . "',
                    '" . $_POST['Total'] . "',
                    '" . date("Y-m-d H:i:s") . "',
        ...
        case 'eliminarCot':
            ...
            $objs = $conn->consulta_simple("DELETE FROM cotizacion WHERE id = " . $_POST["id"]);
            $objs2 = $conn->consulta_simple("DELETE FROM detalles_cotizacion WHERE id_coti = " . $_POST["id"]);
