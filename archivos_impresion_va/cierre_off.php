...
if(isset($_GET["turno"])){
    ...
    // $sqlNull = "SELECT tipo_movimiento, mc.monto
    //             FROM venta v, venta_medio_pago vm, movimiento_caja mc
    //             WHERE total is null AND v.id = vm.id_venta AND mc.fecha_cierre = '".$_GET["fecha"]."' AND tipo_movimiento LIKE CONCAT('%|', vm.id ,'%') ";
    ...
    $sqlticket = 
            "SELECT v.total
            FROM venta v
            WHERE v.fecha_cierre = '".$fecha_cierre."' AND v.tipo_comprobante = 0 ";
    $sqlboleta = 
            "SELECT v.total
            FROM venta v
            WHERE v.fecha_cierre = '".$fecha_cierre."' AND v.tipo_comprobante = 1 ";
    $sqlfactura = 
            "SELECT v.total
            FROM venta v
            WHERE  v.fecha_cierre = '".$fecha_cierre."' AND v.tipo_comprobante =2 ";
 $sqlticketDiscount = 
    ...
    if(intval($_GET["caja"])>0){
        ...
        $mount .= " AND id_caja = '".$_GET["caja"]."'";
         $sqlticket .= " AND id_caja = '".$_GET["caja"]."'";
         $sqlboleta .= " AND id_caja = '".$_GET["caja"]."'";
         $sqlfactura .= " AND id_caja = '".$_GET["caja"]."'";
...
<hr>
    <table align="center">
        ...
        <tr>
            <td>Total Vendido</td>
            <td class="precio"><?php echo number_format($total_vendido-$descuento  + $cobro,2);?></td>
        </tr>
        <tr>
            <td>Efectivo</td>
