    ...
    if(!is_array($test)){
        switch ($_POST['op']) {
            ...
            case 'add':
                ...
                $queryCierre = "SELECT fecha_cierre FROM configuracion";
                $ciere = $objturno->consulta_arreglo($queryCierre);
                ...
                try {
                    ...
                    $fecha_cierre = $ciere["fecha_cierre"];
            ...
            case 'delete_item':
                ...
                $id_secundario = $_POST['id_secundario'];
                ...
                try {
                    ...
                    */
                    ...
                    if ($_POST['is_product'] == 'true') {
                        /** Producto Venta */
                        if ($id_secundario == "undefined") {
                            $queryProductoVenta = "SELECT id from producto_venta WHERE estado_fila=1 AND id_venta = {$id_venta} AND id_producto = {$id_producto} AND prod_secundario = 0";
                            $producto_venta = $objproducto_venta->consulta_arreglo($queryProductoVenta);

                            $objventa->consulta_simple("DELETE FROM producto_venta WHERE id_venta= " . $id_venta . " AND id_producto=" . $id_producto . " AND prod_secundario= 0");
                        } else {

                            $queryProductoVenta = "SELECT id from producto_venta WHERE estado_fila=1 AND id_venta = {$id_venta} AND id_producto = {$id_producto} AND prod_secundario = {$id_secundario}";
                            $producto_venta = $objproducto_venta->consulta_arreglo($queryProductoVenta);

                            $objventa->consulta_simple("DELETE FROM producto_venta WHERE id_venta= " . $id_venta . " AND id_producto=" . $id_producto . " AND prod_secundario=" . $id_secundario . "");
                        }
                       ...
                       // $obj_prod_venta = new producto_venta();
                        ...
                        $objventa->consulta_simple("DELETE FROM movimiento_producto WHERE tipo_movimiento= ".$producto_venta['id']." AND id_producto=" . $id_producto . " and producto_servicio = 1 ");

                        //$obj_mov_prod = new movimiento_producto();
                        //$obj_mov_prod->setVar('id', $movimiento_producto['id']);
                        //$obj_mov_prod->deleteDB();
                        // $objmovimiento_producto->setVar('id');
                        _deleteReceta($objventa, $movimiento_producto['id'], 1);
