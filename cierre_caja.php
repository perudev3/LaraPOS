...
if (isset($_GET["turno"])) {
    ...
    //echo "tipo_usuario ".$_COOKIE['tipo_usuario']."\n";
    //echo "sql= ".$sql."\n";
    //echo "sqlVenta= ".$sqlVenta."\n";
...
//                     }
//                     if (strpos($mv4["tipo_movimiento"], "PLIN_COBRO") !== FALSE) {
//                         $cobro_plin = $cobro_plin + floatval($mv4["monto"]);
//                     }
//                 }
...
//                 }
//                 if ((strpos($mv4["tipo_movimiento"], "PLIN") !== FALSE) && (strpos($mv4["tipo_movimiento"], "SELL") !== FALSE)) {
//                     $plin = $plin + floatval($mv4["monto"]);
//                 }
//             }
//         }
//     }
...
if (is_array($movimientos)) {
    foreach ($movimientos as $mv) {
        ...
        if (strpos($mv["tipo_movimiento"], "_COBRO") !== FALSE) {
            ...
            if (strpos($mv["tipo_movimiento"], "EXT|PEN|VISA_COBRO") !== FALSE) {
                $cobro_visa += floatval($mv["monto"]);
            }
            if (strpos($mv["tipo_movimiento"], "EXT|PEN|MASTERCARD_COBRO") !== FALSE) {
                $cobro_mastercard += floatval($mv["monto"]);
            }
            if (strpos($mv["tipo_movimiento"], "EXT|PEN|DEPOSITO_COBRO") !== FALSE) {
                $cobro_deposito += floatval($mv["monto"]);
            }
            if (strpos($mv["tipo_movimiento"], "EXT|PEN|EFECTIVO_COBRO") === 0) {
                $cobro_efectivo += floatval($mv["monto"]);
            }
            if (strpos($mv["tipo_movimiento"], "YAPE_COBRO") !== FALSE) {
                $cobro_yape += floatval($mv["monto"]);
            }
            if (strpos($mv["tipo_movimiento"], "PLIN_COBRO") !== FALSE) {
                $cobro_plin += floatval($mv["monto"]);
            }
        }
        if (strpos($mv["tipo_movimiento"], "_DETRACCION") !== FALSE) {
            ...
            if (strpos($mv["tipo_movimiento"], "PLIN_DETRACCION") !== FALSE) {
                $detraccion_plin += floatval($mv["monto"]);
            }
        }

        if ((strpos($mv["tipo_movimiento"], "SELL|PEN|EFECTIVO") === 0)) {
            //echo $efectivo." ---";
        ...
        if ((strpos($mv["tipo_movimiento"], "PLIN") !== FALSE) && (strpos($mv["tipo_movimiento"], "SELL") !== FALSE)) {
            $plin += floatval($mv["monto"]);
        }
        ...
        if (strpos($mv["tipo_movimiento"], "BUY|EXT|") !== FALSE) {
            $pruebacotos = $pruebacotos . "|" . $mv["monto"];
            $salidas_cobro = $salidas_cobro + floatval($mv["monto"]);
        }
    }
    //echo "cobro ".$venta;
    //echo "cobro ".$descuento;
    //echo "cobro ".$credito;
    $total_vendido = $venta - $descuento - $credito -$cobro_efectivo - $cobro_visa - $cobro_mastercard;
    //$efectivo =  $total_vendido-$visa-$master-$detraccion_efectivo;
    //$efectivo =  $total_vendido - $visa - $master - $detraccion_efectivo - $yape - $plin;
...
<div class='control-group col-md-6'>
    ...
    <select <?php if ($_COOKIE['tipo_usuario'] > 2) echo 'disabled' ?> class='form-control' id='turno' name='turno'>
        ...
        ?>
                <option value='<?php echo $tututu["id"]; ?>' <?php if (isset($_GET["turno"])) {
                                                                if ($_GET["turno"] == $tututu["id"]) {
                                                                    echo "SELECTED";
                                                                }
                                                            } ?>><?php echo $tututu["nombre"] ?></option>
        <?php
            endforeach;
        endif;
        ?>
...
<div class='control-group col-md-6'>
    ...
    <select <?php if ($_COOKIE['tipo_usuario'] > 2) echo 'disabled' ?> class='form-control' id='caja' name='caja'>
        ...
        ?>
                <option value='<?php echo $tututu["id"] ?>' <?php
                                                            if ($_COOKIE['tipo_usuario'] > 2) {
...
<div class='control-group col-md-6'>
    <label>Vendido en Plin</label>
    <input class='form-control' readonly value="<?php echo number_format($plin - $detraccion_plin, 2); ?>" id="yape" />
</div>
<div class='control-group col-md-6'>
...
<div class='control-group col-md-6' style="display: none">
    <!-- <label>Total Pagado en Fondos YAPE</label> -->
    <label>Pago de Creditos en PLIN</label>
    <input class='form-control' readonly value="<?php echo number_format($cobro_plin, 2); ?>" id="totalFCYAPE" />
</div>
<div class='control-group col-md-6'>
            ...
            <script>
                ...
                function imprimir() {
                    ...
                    $.post('ws/caja.php', {
                        ...
                        console.log(data);
                        if (data !== 0) {
                            document.title = 'USQAY POS IMPRIMIENDO';
